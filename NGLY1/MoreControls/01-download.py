...
inFile = open('SampleInfo3')
ouFile = open('SampleInfo3.txt', 'w')
ouFile2 = open('03-download.sh', 'w')
for line in inFile:
    line = line.strip()
    fields = line.split()
    gsm = fields[0]
    if gsm[0] != '#':
        ...
        for x in L:
            ...
            ouFile2.write('wget ftp://ftp-trace.ncbi.nih.gov/sra/sra-instant/reads/ByRun/sra/SRR/%s/%s/%s.sra\n'%(x[0:6],x,x))
