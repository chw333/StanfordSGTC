...
for ch in CH:
    ...
    if n == 1:
        ouFile.write('\t'.join([head[0], head[1], head[2], head[sI[0]], head[sI[1]], head[sI[2]], head[sI[3]], 'p-value']) + '\n')
    for line in inFile:
        line = line.strip()
        fields = line.split('\t')
        ...
        if pv < 0.01:
            ...
            ouFile.write('\t'.join([fields[0], fields[1], fields[2], '%.2f'%v1, '%.2f'%v2, '%.2f'%v3, '%.2f'%v4, str(pv)]) + '\n')
    inFile.close()
