...
def GetSampleSize(s):
    SampleSizeMedian = []
    SampleSize = {}
    inFile = open('mTECs-Sample-NF-LibrarySize')
    for line in inFile:
        line = line.strip()
        fields = line.split('\t')
...
for i in range(8):
    print('\t'.join([str(x) for x in TL[i]]))
...
for i in range(1):
    ...
    AX.append(sns.kdeplot(np.array(X), shade=False, color=LineColor(i), legend=False, bw=0.5))
#legAX = [AX[0][0], AX[2][0], AX[4][0], AX[6][0]]

#ax.legend(legAX, [Sample2[0], Sample2[2], Sample2[4], Sample2[6]], loc='upper right', prop={'size':12})

plt.savefig('mTECs-TSS-norm-density.pdf')
'''
