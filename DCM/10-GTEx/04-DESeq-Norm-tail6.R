...
Counts = Counts[1:nrow(Counts), (ncol(Counts)-5):ncol(Counts)]
sampleAnnot = sampleAnnot[(nrow(sampleAnnot)-5):nrow(sampleAnnot),]
...
write.table(Count.norm, file="GTEx_Heart_S635A_TwoCtrl_exp-Normalized_tail6.txt",quote=F,sep="\t",row.names=T,col.names=NA)
...
write.table(rlg, file="GTEx_Heart_S635A_TwoCtrl_exp-rlog_tail6.txt",quote=F,sep="\t",row.names=T,col.names=NA)
