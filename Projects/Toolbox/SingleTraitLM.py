...
def SingleTraitLM(inF1, inF2, ouF):
    ...
    for p_ID in phenotype_ID[0:-1]:
        ...
        for i in xrange(pvt.shape[0]):
            p = pvt[i,0]
            if p < SIG:
                ouFile.write('\t'.join([position['chrom'][i], str(position['pos'][i]), '%.4f'%p, p_ID]) + '\n')
    ouFile.close()

SingleTraitLM('Genotype.hdf5','Phenotype.hdf5','Single-Trait-lm-Sig')
