...
def permutation(inF, pv, inD):
    ...
    ouFile = open(inF.split('.txt')[0] + '.WE.txt', 'w')
    inFile = open(inF)
    head = inFile.readline().strip()
    ouFile.write(head + '\tPermutation-pvalue-WE' + '\n')
    for line in inFile:
        line = line.strip()
        fields = line.split('\t')
        phenotype = fields[0] + '-mean'
...
permutation('Yeast-QTL-lm-Sig-0.001.permutation.NA.SA.WA.txt', 'WE-untransformed-pvalue', '/mnt/larsix/projects/NMD/hansun/Projects/YeastQTL/03-linear-regression-noOutlierSD-Covars/GenotypeWE/Permutation')
