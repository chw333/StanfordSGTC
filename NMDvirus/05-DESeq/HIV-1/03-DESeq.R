library(DESeq2)
geneCounts = read.table('HIV1-GeneCount', header=T)
sampleAnnot = read.table('SampleAnnot.txt', header=T)
#which = sampleAnnot$Time == '12h'
sampleAnnot$Condition = relevel(sampleAnnot$Condition, 'Mock')
#sampleAnnot = droplevels(sampleAnnot[which,])
#geneCounts = geneCounts[,wh]

dds = DESeqDataSetFromMatrix(geneCounts, sampleAnnot, design=~Time+Condition)
dds = DESeq(dds)
res = results(dds)
...
save(dds, rld, res, res.sig, res.sig.proteincoding, sampleAnnot, file='resHIV1.rda')

write.table( res, file="deHIV1.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig, file="deHIV1_sig.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding, file="deHIV1_sig_proteincoding.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding.up, file="deHIV1_sig_proteincoding_up.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding.down, file="deHIV1_sig_proteincoding_down.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.nonproteincoding, file="deHIV1_sig_nonproteincoding.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
