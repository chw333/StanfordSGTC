...
wh = grepl('HRSV0h|HRSV4h',sampleAnnot$Condition)
...
save(dds, rld, res, res.sig, res.sig.proteincoding, sampleAnnot, file='resHRSV4h.rda')

write.table( res, file="deHRSV4h.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig, file="deHRSV4h_sig.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding, file="deHRSV4h_sig_proteincoding.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding.up, file="deHRSV4h_sig_proteincoding_up.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding.down, file="deHRSV4h_sig_proteincoding_down.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.nonproteincoding, file="deHRSV4h_sig_nonproteincoding.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
...
write.table(cn, file="geneCounts-HRSV4h-Normalized.txt",quote=F,sep="\t",row.names=T,col.names=NA)
...
colnames(cn)[1]='gene_symbol'
write.table(cn, file="geneCounts-HRSV4h-raw.txt",quote=F,sep="\t",row.names=T,col.names=NA)
