...
wh = grepl('HRSV0h|HRSV16h',sampleAnnot$Condition)
...
save(dds, rld, res, res.sig, res.sig.proteincoding, sampleAnnot, file='resHRSV16h.rda')

write.table( res, file="deHRSV16h.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig, file="deHRSV16h_sig.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding, file="deHRSV16h_sig_proteincoding.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding.up, file="deHRSV16h_sig_proteincoding_up.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding.down, file="deHRSV16h_sig_proteincoding_down.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.nonproteincoding, file="deHRSV16h_sig_nonproteincoding.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
...
write.table(cn, file="geneCounts-HRSV16h-Normalized.txt",quote=F,sep="\t",row.names=T,col.names=NA)
...
colnames(cn)[1]='gene_symbol'
write.table(cn, file="geneCounts-HRSV16h-raw.txt",quote=F,sep="\t",row.names=T,col.names=NA)
