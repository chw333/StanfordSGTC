...
wh = grepl('Mock|HSV8h',sampleAnnot$Condition)
...
save(dds, rld, res, res.sig, res.sig.proteincoding, sampleAnnot, file='resHSV8h.rda')

write.table( res, file="deHSV8h.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig, file="deHSV8h_sig.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding, file="deHSV8h_sig_proteincoding.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding.up, file="deHSV8h_sig_proteincoding_up.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding.down, file="deHSV8h_sig_proteincoding_down.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.nonproteincoding, file="deHSV8h_sig_nonproteincoding.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
...
write.table(cn, file="geneCounts-HSV8h-Normalized.txt",quote=F,sep="\t",row.names=T,col.names=NA)
...
colnames(cn)[1]='gene_symbol'
write.table(cn, file="geneCounts-HSV8h-raw.txt",quote=F,sep="\t",row.names=T,col.names=NA)
