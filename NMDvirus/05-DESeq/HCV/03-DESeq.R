...
save(dds, rld, res, res.sig, res.sig.proteincoding, sampleAnnot, file='resHCV2.rda')
...
write.table( res.sig.proteincoding.up, file="deHCV2_sig_proteincoding_up.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding.down, file="deHCV2_sig_proteincoding_down.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.nonproteincoding, file="deHCV2_sig_nonproteincoding.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
...
write.table(cn, file="geneCounts2-Normalized.txt",quote=F,sep="\t",row.names=T,col.names=NA)
...
colnames(cn)[1]='gene_symbol'
write.table(cn, file="geneCounts2-raw.txt",quote=F,sep="\t",row.names=T,col.names=NA)
