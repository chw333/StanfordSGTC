...
dds = DESeqDataSetFromMatrix(geneCounts, sampleAnnot, design=~Type + Condition)
dds = DESeq(dds)
res = results(dds)
...
save(dds, rld, res, res.sig, res.sig.proteincoding, sampleAnnot, file='resHCV.rda')

write.table( res, file="deHCV.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig, file="deHCV_sig.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding, file="deHCV_sig_proteincoding.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding.up, file="deHCV_sig_proteincoding_up.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding.down, file="deHCV_sig_proteincoding_down.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.nonproteincoding, file="deHCV_sig_nonproteincoding.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
