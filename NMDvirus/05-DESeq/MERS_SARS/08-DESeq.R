...
wh = grepl('MOCK|SARS_MRC5HighMOI_48hr', sampleAnnot$Sample) 
...
save(dds, rld, res, res.sig, res.sig.proteincoding, sampleAnnot, file='resSARS_high_48h.rda')

write.table( res, file="deSARS_high_48h.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig, file="deSARS_high_48h_sig.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding, file="deSARS_high_48h_sig_proteincoding.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding.up, file="deSARS_high_48h_sig_proteincoding_up.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding.down, file="deSARS_high_48h_sig_proteincoding_down.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.nonproteincoding, file="deSARS_high_48h_sig_nonproteincoding.txt", quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
...
write.table(cn, file="SARS_high_48h_geneCounts-Normalized.txt",quote=F,sep="\t",row.names=T,col.names=NA)
...
colnames(cn)[1]='gene_symbol'
write.table(cn, file="SARS_high_48h_geneCounts-raw.txt",quote=F,sep="\t",row.names=T,col.names=NA)
